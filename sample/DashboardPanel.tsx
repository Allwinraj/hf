...
function MailReplyTable({
  ...
  const [sapPopup, setSapPopup] = useState<Record<string, unknown>[] | null>(null)
  ...
  const postSap = async (mailId?: string) => {
    if (!runId) return
    setBusy(mailId ? `sap-${mailId}` : 'sap_all')
    try {
      const result = await api.postSap(runId, mailId ? { mail_ids: [mailId] } : { post_all: true })
      setSapPopup(result.results || [])
      await onMailChange?.()
    } catch (err: unknown) {
      setSapPopup([
        {
          ok: false,
          PurchaseOrder: '',
          error: err instanceof Error ? err.message : 'SAP post failed',
        },
      ])
    } finally {
      setBusy(null)
    ...
    <div className="neu-raised lift col-span-full flex max-h-[40rem] min-h-0 flex-col overflow-hidden rounded-2xl p-4">
      <div className="mb-3 flex shrink-0 items-center justify-between gap-2">
        <div>
          ...
          <p className="mt-1 font-mono-label text-on-surface-variant">
            Review the draft, then Send mail or Post in SAP.
          </p>
        </div>
        <div className="flex gap-2">
          <button
            className="rounded-xl border border-[var(--ey-line)] px-3 py-1.5 font-label-md disabled:opacity-50"
            disabled={!runId || busy != null}
            onClick={() => void postSap()}
          >
            {busy === 'sap_all' ? 'Posting…' : 'Post all in SAP'}
          </button>
          <button
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        <table className="data-table w-full text-left font-body-md text-sm">
          <thead className="sticky top-0 z-10 bg-surface">
            <tr>
              {['subject', 'item', 'qty', 'amount', 'verdict', 'status', 'SAP PO', ''].map((col) => (
                <th
                  key={col}
                  className="border-b border-[var(--ey-line)] bg-surface pb-2 pr-3 font-mono-label uppercase tracking-wider text-on-surface-variant"
                >
              ...
              return (
                <Fragment key={mailId || String(row.subject)}>
                  ...
                  >
                    ...
                    <td className="py-2 pr-3 text-on-surface">{String(row.qty ?? '')}</td>
                    <td className="py-2 pr-3 text-on-surface">{String(row.budget ?? '')}</td>
                    <td className="py-2 pr-3 text-on-surface">{String(row.verdict ?? '')}</td>
                    <td className="py-2 pr-3 text-on-surface">{status}</td>
                    <td className="py-2 pr-3 text-on-surface">{String(row.sap_purchase_order ?? '')}</td>
                    <td className="py-2 pr-3">
                      <div className="flex flex-wrap gap-2" onDoubleClick={(event) => event.stopPropagation()}>
                        {status === 'draft' && runId ? (
                          <>
                            <button
                        ...
                        {runId && status !== 'skipped' ? (
                          <button
                            className="rounded-lg border border-[var(--ey-line)] px-2 py-1 font-mono-label disabled:opacity-50"
                            disabled={busy != null}
                            onClick={() => void postSap(mailId)}
                          >
                            {busy === `sap-${mailId}` ? 'Posting…' : 'Post in SAP'}
                          </button>
                        ) : null}
                      </div>
                    </td>
      ...
      {sapPopup ? <SapOrderPopup results={sapPopup} onClose={() => setSapPopup(null)} /> : null}
    </div>
  )
}

function SapOrderPopup({
  results,
  onClose,
}: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div
        className="neu-raised w-full max-w-lg rounded-2xl bg-surface p-5"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="font-headline-sm text-accent-grad">SAP purchase orders</div>
        <ul className="mt-3 space-y-2">
          {results.map((item, index) => {
            const number = String(item.PurchaseOrder ?? '')
            const ok = Boolean(item.ok) || Boolean(item.skipped)
            return (
              <li key={String(item.mail_id ?? index)} className="neu-inset rounded-xl px-3 py-2 font-body-md text-sm">
                {number ? (
                  <span>
                    PurchaseOrder: <span className="font-semibold">{number}</span>
                    {item.skipped ? ' (already posted)' : ''}
                  </span>
                ) : (
                  <span className="text-error">{String(item.error || 'No PurchaseOrder in the SAP response')}</span>
                )}
                {!ok && number ? <p className="mt-1 text-error">{String(item.error || '')}</p> : null}
              </li>
            )
          })}
        </ul>
        <div className="mt-4 flex justify-end">
          <button className="cta-sheen rounded-xl bg-primary-container px-3 py-2 font-label-md font-bold" onClick={onClose}>
            Close
          </button>
        </div>
      </div>
...
function MailDraftEditor({
  ...
  const [itemText, setItemText] = useState(String(row.item ?? ''))
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <label className="block">
          <span className="mb-1 block font-mono-label text-on-surface-variant">OrderQuantity</span>
          <input className="field" value={qty} onChange={(e) => setQty(e.target.value)} />
        </label>
        <label className="block">
          <span className="mb-1 block font-mono-label text-on-surface-variant">NetPriceAmount</span>
          <input className="field" value={amount} onChange={(e) => setAmount(e.target.value)} />
        </label>
        <label className="block">
          <span className="mb-1 block font-mono-label text-on-surface-variant">PurchaseOrderItemText</span>
          <input className="field" value={itemText} onChange={(e) => setItemText(e.target.value)} />
        </label>
      </div>
      <label className="block">
