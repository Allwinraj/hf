...
def is_placeholder_reason(text: str) -> bool:
    blob = (text or "").lower()
    return any(
        token in blob
        for token in (
            "a complete policy explanation was not stored",
            "could not complete a model verdict",
            "treat the decision line above",
        )
    )


def synthesize_mail_explanation(item: dict[str, Any]) -> str:
    ...
    worth = f", value {value}," if value else ""
    request = f"The purchase request for {qty_bit}{goods}{where}{worth}".replace(" ,", ",")
    missing: list[str] = []
    if not item.get("cost_centre"):
        missing.append("cost centre")
    if not item.get("needed_by"):
        missing.append("needed-by date")
    if missing:
        gap = " Missing from the request: " + ", ".join(missing) + "."
    else:
        gap = ""
    if verdict == "approved":
        return (
            f"{request} is approved under the uploaded purchase policy based on the details "
            "in the request and any attached specification."
        )
    if verdict == "escalated":
        return (
            f"{request} is escalated under the uploaded purchase policy and is not approved. "
            "A higher authority must review it."
            + gap
        )
    return (
        f"{request} is flagged under the uploaded purchase policy and is not approved."
        + gap
    )


def fill_mail_explanation(item: dict[str, Any]) -> dict[str, Any]:
    out = dict(item)
    why = str(out.get("explanation") or out.get("_decision_explanation") or "").strip()
    if not why or is_placeholder_reason(why):
        out["explanation"] = synthesize_mail_explanation(out)
    if is_placeholder_reason(str(out.get("remediation") or "")):
...
def flatten_mail_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return fill_mail_explanation(merged)
...
def _draft_reply_body(item: dict[str, Any], send_details: str = "") -> str:
    ...
    if not why or is_placeholder_reason(why):
        why = synthesize_mail_explanation(item)
    remediation = str(item.get("remediation") or "").strip()
    if is_placeholder_reason(remediation):
        remediation = ""
    qty_bit = f"{qty} " if qty not in (None, "") else ""
...
def _compact_mail_record(item: dict[str, Any]) -> dict[str, Any]:
    ...
    if is_placeholder_reason(str(out.get("explanation") or "")):
...
async def _one_llm_draft(llm: Any, item: dict[str, Any], send_details: str) -> dict[str, Any]:
    item = fill_mail_explanation(flatten_mail_row(item))
    ...
    prompt = (
        ...
        "Never write that a policy explanation was not stored.\n"
...
def collect_mail_rows(run: Any) -> list[dict[str, Any]]:
    ...
    for mail_id, row in by_id.items():
        ...
        if mail_id not in drafts and status == "draft":
            item = fill_mail_explanation(item)
            body = str(item.get("draft_body") or "")
            stale = (
                not body
                or is_placeholder_reason(body)
                or "cannot auto-approve" in body.lower()
                or "dashboard" in body.lower()
                or not _draft_looks_complete(body)
            )
            if stale and (item.get("verdict") or item.get("explanation")):
        ...
        sap = dict(extra.get("mail_sap") or {}).get(mail_id) or {}
        if isinstance(sap, dict) and sap.get("PurchaseOrder"):
            item["sap_purchase_order"] = sap["PurchaseOrder"]
            item["sap_status"] = "posted"
        rows.append(item)
    run.extra = extra
