...
class Decision:
    ...
    async def execute(self, ctx: RunContext, env: Envelope) -> list[Envelope]:
        ...
        if llm_indexes:
            try:
                await asyncio.wait_for(
                    asyncio.gather(
                        *(_judge(i, records[i]) for i in llm_indexes),
        ...
        from app.services.mail import attach_drafts, attach_drafts_llm, fill_mail_explanation, flatten_mail_row

        flattened: list[tuple] = []
        for port, row in items:
            flattened.append((port, fill_mail_explanation(flatten_mail_row(row))))
        if any(row.get("mail_id") for _, row in flattened):
            try:
                ...
                drafted = attach_drafts([row for _, row in flattened])
                flattened = [(flattened[i][0], drafted[i]) for i in range(len(flattened))]
        items = flattened
